...
public Trajectory[] path1 = new Trajectory[1];


   DriveSubsystem m_drive = new DriveSubsystem();
   public ScorpTrajectory(DriveSubsystem m_drive) {
     this.m_drive = m_drive;
//(max velocity,max acceliration)

TrajectoryConfig trajconf = new TrajectoryConfig(Constants.ODOMETRY.kMaxSpeedMetersPerSecond, Constants.ODOMETRY.kMaxAccelerationMetersPerSecondSquared).setKinematics(m_drive.getKinematic());



  path1[0] = TrajectoryGenerator.generateTrajectory(
